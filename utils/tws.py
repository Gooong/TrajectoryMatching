...
def tws(record1, record2):
    ...
    delta_time = list(map(operator.sub, common_time[1:], common_time))
    final_delta_time = list(map(operator.add, itertools.chain([0], delta_time), itertools.chain(delta_time, [0])))
    score = sum(map(operator.mul, final_delta_time, similarity_list)) / 2
    return score
